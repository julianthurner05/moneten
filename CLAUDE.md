...
5. **Einkaufsliste** (Tab „Einkaufsliste", nur bei `kind = 'wg'`, probeweise): gemeinsame Liste pro Gruppe (`shopping_items`). Artikel oben über ein Eingabefeld ergänzen, Zeilen zum Abhaken antippen (runder Haken links), Erledigtes steht gesondert darunter und lässt sich gesammelt löschen.
6. **Web-Push-Grundstein:** Service Worker (`public/sw.js`), Abo-Verwaltung (`push_subscriptions`, `/api/push/subscribe`), VAPID-Public-Key in `js/push.js` (privater Schlüssel liegt nicht im Repo). Aktivieren/Deaktivieren über einen Button in der Budgetplanung; Versandlogik in `shared/webpush.js` (VAPID + RFC-8291-Verschlüsselung) mit Test-Endpunkt `/api/push/test`; der private Schlüssel muss als Secret `VAPID_PRIVATE_KEY` gesetzt sein (liegt lokal in `~/.moneten-vapid-private.txt`). Automatische Auslöser (`shared/notify.js`, fehlertolerant per `waitUntil`): neue Gruppenausgabe → Beteiligte außer der eintragenden Person; neue unbestätigte Begleichung → empfangende Person; Bestätigung → zahlende Person. Gilt auch für den Einzug-Bereich (eigener Titel und Link).
7. **Bestätigung von Ausgleichszahlungen:** Eine Zahlung zählt erst in die Salden, wenn die empfangende Person sie bestätigt hat (`settlements.confirmed_at`). Trägt die empfangende Person sie selbst ein, gilt sie sofort als bestätigt. Unbestätigte Zahlungen sind markiert („Wartet auf Bestätigung") mit „Erhalten"-Button für die empfangende Person. Keine Bank-Anbindung.
