...
6. **Web-Push-Grundstein:** Service Worker (`public/sw.js`), Abo-Verwaltung (`push_subscriptions`, `/api/push/subscribe`), VAPID-Public-Key in `js/push.js` (privater Schlüssel liegt nicht im Repo). Aktivieren/Deaktivieren über einen Button in der Budgetplanung; der Versand von Mitteilungen ist noch nicht gebaut.
